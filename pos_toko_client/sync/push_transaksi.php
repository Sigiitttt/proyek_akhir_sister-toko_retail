...
try {
    // 1. CARI TRANSAKSI PENDING (Belum dikirim)
    $stmt = $db_lokal->query("SELECT * FROM transaksi WHERE is_synced = 0 LIMIT 50"); // Batasi 50 biar ringan
    ...
    if (count($pendingTrx) > 0) {
        ...
        foreach ($pendingTrx as $trx) {
            ...
            $payload[] = [
                ...
                'id_toko'         => ID_TOKO, // Dari config app.php
        ...
        if (isset($response['status']) && $response['status'] == 'success') {
            ...
            echo "Gagal Upload: " . ($response['message'] ?? 'API Error');
        }
